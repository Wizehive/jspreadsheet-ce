...
(function (global, factory) {
  ...
  var jexcel = function (el, options) {
    ...
    obj.prepareTable = function () {
      ...
      // Number of columns
      var size = obj.options.minDimensions[0];
      var rowSize = obj.options.minDimensions[1];
      ...
      for (var rowIndex = 0; rowIndex < rowSize; rowIndex++) {
        ...
        for (var columnIndex = 0; columnIndex < size; columnIndex++) {
          ...
          if (!obj.options.cellDataTypes[rowIndex][columnIndex].width) {
            obj.options.cellDataTypes[rowIndex][columnIndex].width = obj.options
              .colWidths[i]
              ? obj.options.colWidths[i]
              : obj.options.defaultColWidth;
          }
          if (!obj.options.cellDataTypes[rowIndex][columnIndex].align) {
            obj.options.cellDataTypes[rowIndex][columnIndex].align = obj.options
              .colAlignments[i]
              ? obj.options.colAlignments[i]
              : "center";
          }
          if (
            obj.options.cellDataTypes[rowIndex][columnIndex].type == "calendar"
          ) {
            if (
    ...
    obj.createTable = function () {
      ...
      if (
        ...
        obj.options.showToolbar
      ) {
        obj.createToolbar();
      }

      // Create toolbar
      if (obj.options.allowFxbar) {
        obj.createFxBar();
      }
    ...
    obj.createToolbar = function (toolbar) {
      ...
      for (var i = 0; i < toolbar.length; i++) {
        if (
          ...
          var dynamicId = "selectWith";
          toolbarItem.id = dynamicId;
          if (toolbar[i].disable) {
            toolbarItem.setAttribute("data-disabled", "disabled");
          }
          ...
          for (var j = 0; j < toolbar[i].v.length; j++) {
            ...
            toolbarDropdownOption.textContent = toolbar[i].title
              ? toolbar[i].title[j]
              : toolbar[i].v[j];
            if (
          ...
          if (toolbar[i].tooltip) {
            ...
            tooltipDiv.setAttribute("title", toolbar[i].tooltip);
            tooltipDiv.appendChild(tooltipSpan);
            obj.toolbar.appendChild(tooltipDiv);
    ...
    obj.createFxBar = function () {
      // Create the first read-only input box
      ...
      if (cell.classList.contains("readonly") == true) {
        ...
        if (obj.options.columns[x].editor) {
          ...
          if (obj.options.cellDataTypes[y][x].type == "hidden") {
            ...
            var value = obj.options.data[y][x];
            if (
              obj.options.cellDataTypes[y][x].multiple &&
              !Array.isArray(value)
            ) {
              value = value.split(";");
            }
            ...
            obj.options.cellDataTypes[y][x].options.value =
              obj.options.data[y][x];
            obj.options.cellDataTypes[y][x].options.opened = true;
            obj.options.cellDataTypes[y][x].options.onclose = function (
              el,
              value
            ) {
              obj.closeEditor(cell, true);
      ...
      if (scrollLeft > 1) {
        for (var i = 0; i < obj.options.freezeColumns; i++) {
          ...
          obj.headers[i].style.left =
            width + (obj.options.hideRowHeader ? "" : 50) + "px";
